...
import { DownloadIcon, SpinnerIcon, ErrorIcon, LogIcon, UsersIcon, TruckIcon, PlusIcon, TrashIcon, PencilIcon, QRIcon, UsersAdminIcon, SignatureIcon } from './Icons';
...
const QRCodeModal: React.FC<{ passenger: Passenger; onClose: () => void }> = ({ passenger, onClose }) => {
    ...
    const handlePrint = () => {
        ...
        if (printContents) {
            ...
            // Restore functionality by reloading the page.
            ...
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-sm m-4 transform transition-all text-center" onClick={(e) => e.stopPropagation()}>
                <div ref={modalContentRef}>
                    ...
                    <div className="flex justify-center my-4 h-[256px] w-[256px] mx-auto">
                        {qrError 
                            ? <div className="text-red-500 bg-red-100 flex items-center justify-center w-full h-full rounded-lg p-4">Error al generar el código QR.</div>
                            : <canvas ref={qrRef} />
                        }
                    </div>
